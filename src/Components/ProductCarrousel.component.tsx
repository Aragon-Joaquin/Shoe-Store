...
export default function ProductCarrousel({
	...
	function scrollCarrousel(direction: 'left' | 'right') {
		const dirScroll =
			direction === 'left' ? SCROLLTO_SLIDE_PX : -SCROLLTO_SLIDE_PX
					...
					{returnResponse?.map((product) => {
						return (
							...
							>
								<Link
									to={product.title}
									className='h-40 border-b-2 border-b-mainPalette-darkBrown2 mb-2'
								>
									<img
										src={getImage()}
										alt={product.title}
								...
								<div>
									...
									<div>
										<p className='text-lg text-lime-600 font-bold text-center'>
									...
									<Button
										...
										className='text-nowrap w-full m-auto text-center mt-2'
									>
										Add to cart
									</Button>
								</div>
