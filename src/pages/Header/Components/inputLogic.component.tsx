...
export function InputLogic({ quantity, product }: { quantity: number; product: productAdapted }) {
	...
	const controlQuantity = debouncer((...args: unknown[]) => {
		...
		if (current.valueAsNumber + addQuant === 0) return removeCart(product.idProduct)
	...
	return (
		<div className="flex flex-col justify-center items-center px-2">
			<Arrow className="rotate-90 bg-transparent !w-10 hover:scale-125" onClick={() => controlQuantity(1)} />
			<input
				ref={inputRef}
				type="number"
